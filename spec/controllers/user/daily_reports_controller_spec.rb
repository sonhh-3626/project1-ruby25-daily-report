...
RSpec.describe User::DailyReportsController, type: :controller do
  let(:user) { instance_double("User", id: 1, department_id: 1, sent_reports: mock_relation) }
  let(:manager) { instance_double("User", id: 2) }
  let(:daily_report) { instance_double("DailyReport", id: 1) }
  ...
  describe "GET #index" do
    before do
      ...
      allow(controller).to receive(:instance_variable_set).with(:@daily_reports, mock_relation)
      get :index
    end
